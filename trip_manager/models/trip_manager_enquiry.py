...
class TripManagerEnquiry(models.Model):
    ...
    #   MARK: FIELDS
    # ------------------------------------------------------------------------------
    tour_start_date = fields.Date(string="Start date")
    tour_end_date = fields.Date(string="End date")
    ...
    package_from_city   = fields.Char(related='package_id.from_city', string='From City')
    ...
    package_margin      = fields.Float(related='package_id.margin_surplus', string='Margin (%)')
    ...
    customer_id = fields.Many2one(comodel_name='res.partner', string="Customer", required=True, 
                                  change_default=True, index=True, check_company=True)
    ...
    package_id = fields.Many2one('trip.manager.package',  string='Packages', 
                                   domain="[('destination_city_id', '=', destination_city_id)]")
    ...
    def _compute_no_of_day(self):
        """Computes the total number of tour days inclusively from
    ...
    @api.constrains('tour_start_date', 'tour_end_date')
    def _check_tour_dates(self):
        """Validates that tour start date is not in the past and that
        tour end date is not before the start date. Raises a ValidationError
        on violation."""
        
        today = fields.Date.today()
        for rec in self:
            if rec.tour_start_date and rec.tour_start_date < today:
                raise ValidationError(
                    "Tour start date cannot be in the past.")
            if rec.tour_end_date and rec.tour_start_date and rec.tour_end_date < rec.tour_start_date:
                raise ValidationError(
    ...
    #   MARK: BUTTON METHODS
    # ------------------------------------------------------------------------------
    def action_set_estimate(self):
        """Moves the enquiry to Estimate state, indicating that a
